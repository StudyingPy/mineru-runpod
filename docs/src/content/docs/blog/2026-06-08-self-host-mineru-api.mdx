...
The trade-off is honest: you run the infrastructure. That means a cold-start tax on the first request after the worker scales to zero, and a one-time model download on first boot. If your traffic is a handful of pages a month, the cloud API's free tier is simpler. Cross into steady volume and the math flips.

## What does self-hosting MinerU cost vs the cloud API?

Roughly **$0.001 per page** warm on a 24 GB RTX 4090, plus a **~$0.03 fixed tax per cold start** (about 110 seconds of GPU billing for vLLM engine init plus model load). RunPod bills per second and scales to zero, so an idle endpoint costs nothing. The cloud API is free under its daily quota, then queues or meters you; self-hosting trades that ceiling for a steady, predictable per-page rate.

The real number depends on how well you amortize the cold start. A thousand pages parsed across one warm window land near $0.001/page; a single short doc per scale-from-zero lands closer to $0.005–$0.01 because you're paying the fixed tax against very few pages. I broke the full workload-shape math down in [Serverless MinerU on RunPod: honest cost math](/mineru-runpod/blog/2026-05-19-launching-mineru-runpod/).

Two costs the marketing pages tend to skip: the ~110-second cold start is billed GPU time, and the first boot on a fresh image downloads the model (a few minutes, once). Neither is a dealbreaker, but budget for them instead of being surprised.
...
Most of it carries; four things don't. The compat client **rejects** `callback` (a RunPod webhook delivers a different, unsigned payload than MinerU's signed `{checksum, content}` callback, so it raises instead of misleading you), and it doesn't support `extra_formats` (docx/html/latex), the `MinerU-HTML` model, or multi-range `page_ranges` like `"2,4-6"`. Batch isn't wired yet. And `full_zip_url` requires the `BUCKET_*` setup above.
...
| Batch (`/extract/task/batch`) | Not yet; submit tasks individually for now |

Where it falls down: if your pipeline leans on webhook callbacks or exports to DOCX/LaTeX, the compat client isn't a clean swap today. Batch is the next thing on the roadmap. For everything else, the create / poll / download path behaves like the SaaS.
